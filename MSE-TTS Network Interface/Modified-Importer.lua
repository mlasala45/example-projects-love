...
SCRYFALL_NAME_BASE_URL = "http://localhost:12345/?q="
...
local function getAugmentedOracleText(cardData)
    --print("getAugmentedOracleText")
    --print(tostring(cardData.oracle_text))
    --print(tostring(cardData.name))
...
local function parseMTGALine(line)
    ...
    --Original name match string: ([^%(%)]+)
    --If changed, this totally breaks sets with "Set" specifiers, unfortunately
    local nameMatchString = "([^%(%)]+)"

    local name, setCode, collectorNum = string.match(line, nameMatchString.." %(([%d%l%u]+)%) ([%d%l%u]+)")

    if not name then
        name, setCode = string.match(line, nameMatchString.." %(([%d%l%u]+)%)")
    end

    if not name then
       name = string.match(line, nameMatchString)
    end

    if not name then
        print("parseMTGALine failed")
        print(line)
    end
