...
local ProductionQueue = Class(Widget, function(self, x, y, w, h, color1, color2)
	...
	self.bg = self:AddChild(RoundedBox(0, 0, self.w, self.h, thick, self.color1, self.color2))
	...
	local nav_gap = nav_spacing + 40

	self.scroll_bg = self:AddChild(Box(thick, y_acc, iw, ih-nav_gap, 0, COLORS.CLEAR, COLORS.CLEAR))
	self.scroll_1 = self:AddChild(ScrollArea(thick,y_acc,iw,ih-nav_gap))
	self.scroll_2 = self:AddChild(ScrollArea(thick,y_acc,iw,ih-nav_gap))

	self.buildoptions = self.scroll_1:AddChild(MenuList(0, 0, iw, 0, seperation))
	self.buildqueue = self.scroll_2:AddChild(MenuList(0, 0, iw, 0, seperation))
	self.buildoptions.name = "OPTIONS"
	self.buildoptions.scroll_master = self.scroll_1
	self.buildqueue.name = "QUEUE"
	self.buildqueue.scroll_master = self.scroll_2
	...
	local nav_button = self.nav_button
	self.nav_button.ORGANIZER = {
		bg = {
			_inst = self.nav_button:AddChild(RoundedBox(0,0,nav_button.w,nav_button.h,nav_thick, self.color1, self.color2))
		}
	}

	self.is_ui = true --clumsy
	self.nav_txt.is_ui = true
...
function ProductionQueue:RecalcBuildOptions()
	...
	for i,v in ipairs(options) do
		if not ((not v.repeatable) and table.contains(self.city.production_queue, v)) then
			...
			self.scroll_1.scroll_max = math.max(0, self.buildoptions.h - self.scroll_1.h)
		end
	end
end
...
function ProductionQueue:RecalcBuildQueue()
	...
	for i,v in ipairs(elements) do
		...
		self.scroll_2.scroll_max = math.max(0, self.buildqueue.h - self.scroll_2.h)
	end
end
