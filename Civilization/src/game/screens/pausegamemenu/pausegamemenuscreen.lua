...
local PauseGameMenuScreen = Class(Screen, function(self)
	Screen._ctor(self)

	self.name = "PauseGameMenuScreen"
...
function PauseGameMenuScreen:AddButton(i, str)
...
function PauseGameMenuScreen:OnButtonClicked(i)
	if i == Enum_PGM_Buttons.Resume then
		self:Hide()
	elseif i == Enum_PGM_Buttons.LoadGame then
		TheSaveGameIndex:LoadGameFromFile("quicksave")
	elseif i == Enum_PGM_Buttons.SaveGame then
		TheSaveGameIndex:SaveGameToFile("quicksave")
